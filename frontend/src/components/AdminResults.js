...
function AdminResults() {
    ...
    const fetchResponseDetails = (responseId, quizId) => {
        axios
            .get(`http://localhost:5000/api/response-details/${responseId}?quizId=${quizId}`)
            .then((response) => {
    ...
    const handleSubmitQuiz = () => {
        axios
            .post('http://localhost:5000/api/quizzes', newQuiz)
            .then(() => {
                alert('Quiz created successfully!');
                setNewQuiz({ title: '', description: '', questions: [] });
                axios.get('http://localhost:5000/api/quizzes').then((response) => setQuizzes(response.data));
            })
